...
export function validateReportHtml(html) {
    ...
    if (!html.includes('report-header')) {
        ...
        const missing = [];
        if (!html.includes('hdr-brand'))
            missing.push('hdr-brand');
        if (!html.includes('hdr-title'))
            missing.push('hdr-title');
        if (!html.includes('hdr-sub'))
            missing.push('hdr-sub');
        if (!html.includes('hdr-meta'))
            missing.push('hdr-meta');
        if (missing.length > 0) {
